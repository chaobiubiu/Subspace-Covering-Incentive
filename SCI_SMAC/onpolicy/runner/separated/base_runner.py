...
class Runner(object):
    def __init__(self, config):
        ...
        elif self.all_args.algorithm_name == "sci":
            from onpolicy.algorithms.sci.sci_trainer import SCITrainer as TrainAlgo
            from onpolicy.algorithms.sci.algorithm.SCIPolicy import SCIPolicy as Policy
        else:
        ...
        # Count the state
        if self.all_args.scenario_name == 'pass' or self.all_args.scenario_name == 'no_door_pass':
            # self.state_count = np.zeros((np.prod((30, 30, 30, 30, 2))))
            self.state_count = np.zeros((30, 30, 30, 30))
        else:
    ...
    def compute(self):
        for agent_id in range(self.num_agents):
            if self.state_count is not None:
                self.buffer[agent_id].global_novelty *= self.all_args.bonus_coef

...
from onpolicy.runner.shared_count.base_runner import Runner
...
class FootballRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if total_num_steps % self.log_interval == 0:
                ...
                self.log_hdd_train(hdd_train_infos, total_num_steps)
    ...
    def log_hdd_train(self, hdd_train_infos, total_num_steps):
        for k, v in hdd_train_infos.items():
            if self.use_wandb:
    ...
    def eval(self, total_num_steps):
        ...
        if self.use_wandb:
            ...
            self.writter.add_scalar("eval_goal", eval_goal, total_num_steps)
            self.writter.add_scalar("eval_win_rate", eval_win_rate, total_num_steps)
            self.writter.add_scalar("eval_step", eval_step, total_num_steps)
    ...
    def render(self):        
        ...
        for i_episode in range(self.all_args.render_episodes):
            render_obs, render_share_obs, render_available_actions = render_env.reset()
            ...
            while not np.any(render_dones):
                ...
                # step
                render_obs, render_share_obs, render_rewards, render_dones, render_infos, render_available_actions = render_env.step(render_actions_env)

...
class ShareVecEnv(ABC):
    ...
    def __init__(self, num_envs, observation_space, share_observation_space, action_space, nf_al=None, nf_en=None):
...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send((env.observation_space, env.share_observation_space, env.action_space, env.nf_al, env.nf_en))
        else:
            raise NotImplementedError
...
class SubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.remotes[0].send(('get_spaces', None))
        observation_space, share_observation_space, action_space, nf_al, nf_en = self.remotes[0].recv()
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
                             share_observation_space, action_space, nf_al, nf_en)
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send(
                (env.observation_space, env.share_observation_space, env.action_space, env.nf_al, env.nf_en))
...
class ShareDummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns):
        ...
        env = self.envs[0]
        ShareVecEnv.__init__(self, len(
            env_fns), env.observation_space, env.share_observation_space, env.action_space, env.nf_al, env.nf_en)
        self.actions = None

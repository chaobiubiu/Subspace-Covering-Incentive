import numpy as np
import copy
from onpolicy.envs.grid.rooms import Rooms, Entity, Door
import gym
TOP = 0
BOT = 1
LEFT = 2
...
class SecretRooms(Rooms):
    def __init__(self, H=300, grid_size=25, n_actions=4, n_agents=2, checkpoint=False):
        ...
        # (x1, y1, x2, y2, (door1_opened, door2_opened, door3_opened)), 2^{3} kinds of status of three doors.
        ...
        half = grid_size // 2
        one = grid_size
        door1_y, door2_y, door3_y = onethrid // 2, (onethrid + twothrid) // 2, (twothrid + one) // 2        # Three doors have the same x info, but different y info
        point8, point2 = int(grid_size * 0.8), int(grid_size * 0.2)     # 20, 5
        ...
        self.switches = [Entity(point2, point8), Entity(point8, door1_y),
                         Entity(point8, door2_y), Entity(point8, door3_y)]      # Switch 1, 2, 3, and 4.
    ...
    def _reward(self):
        ...
        for agent in self.agents:
            if agent.x < (self.grid_size / 2 + 1) or agent.y > (self.grid_size // 3 + 1):       # If any agent do not lie in the upper right room, return 0 team reward.
                return rew

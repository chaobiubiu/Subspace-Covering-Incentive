...
class HindsightReplayBuffer(SharedReplayBuffer):
    """
    Buffer to store training data.
    :param args: (argparse.Namespace) arguments containing relevant model, policy, and env information.
    :param num_agents: (int) number of agents in the env.
    ...
    """
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        super(HindsightReplayBuffer, self).__init__(args, num_agents, obs_space, cent_obs_space, act_space)
        ...
        self.sub_states = np.zeros((self.num_objects, self.episode_length + 1, self.n_rollout_threads, num_agents, *share_obs_shape), dtype=np.float32)

        # Below novels are shared among agents.
        self.state_novels = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.sub_state_novels = np.zeros((self.num_objects, self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)

        self.ratios = np.zeros((self.num_objects, self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.hdd_advantages = np.zeros((self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)

    def compute_returns(self, next_value, value_normalizer=None, state_novel_coef=0.0, ir_coef=0.0):
        """
        Compute returns either as discounted sum of rewards, or using GAE.
        :param next_value: (np.ndarray) value predictions for the step after the last episode step.
        :param value_normalizer: (PopArt) If not None, PopArt value normalizer instance.
        ...
        if self._use_proper_time_limits:
            if self._use_gae:
                ...
                gae = 0
                for step in reversed(range(self.rewards.shape[0])):
                    proxy_rewards = self.rewards[step].copy()
                    proxy_rewards += state_novel_coef * self.state_novels[step + 1] * self.masks[step + 1]
                    proxy_rewards += ir_coef * self.hdd_advantages[step]

                    if self._use_popart or self._use_valuenorm:
                        # step + 1
                        delta = proxy_rewards + self.gamma * value_normalizer.denormalize(
                            self.value_preds[step + 1]) * self.masks[step + 1] \
                                - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * gae * self.masks[step + 1]
                        gae = gae * self.bad_masks[step + 1]
                        ...
                        delta = proxy_rewards + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - \
                                self.value_preds[step]
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        gae = gae * self.bad_masks[step + 1]
                        self.returns[step] = gae + self.value_preds[step]
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    ...
                    if self._use_popart or self._use_valuenorm:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + proxy_rewards) * self.bad_masks[step + 1] \
                                             + (1 - self.bad_masks[step + 1]) * value_normalizer.denormalize(
                            self.value_preds[step])
                    else:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + proxy_rewards) * self.bad_masks[step + 1] \
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    ...
                    if self._use_popart or self._use_valuenorm:
                        delta = proxy_rewards + self.gamma * value_normalizer.denormalize(self.value_preds[step + 1]) * self.masks[step + 1] \
                                - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
                    else:
                        delta = proxy_rewards + self.gamma * self.value_preds[step + 1] * \
                                self.masks[step + 1] - self.value_preds[step]
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + self.value_preds[step]
            else:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    ...
                    self.returns[step] = self.returns[step + 1] * self.gamma * self.masks[step + 1] + proxy_rewards

    def hdd_generator(self, object_index, num_mini_batch=None, mini_batch_size=None):
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        batch_size = n_rollout_threads * episode_length * num_agents
        ...
        curr_substates = self.sub_states[object_index, 1:].reshape(-1, *self.sub_states.shape[4:])
        obs = self.obs[:-1].reshape(-1, *self.obs.shape[3:])
        actions = self.actions.reshape(-1, self.actions.shape[-1])
        if self.available_actions is not None:
            available_actions = self.available_actions[:-1].reshape(-1, self.available_actions.shape[-1])
        ...
        for indices in sampler:
            substates_batch = curr_substates[indices]
            obs_batch = obs[indices]
            actions_batch = actions[indices]
            if available_actions is not None:
                available_actions_batch = available_actions[indices]
            ...
            yield obs_batch, actions_batch, substates_batch, available_actions_batch, active_masks_batch
...
class HindsightBuffer(object):
    def __init__(self, args, obs_space, cent_obs_space, act_space):
        ...
        self.cent_obs = np.zeros((self.length, self.n_rollout_threads, cent_obs_shape[0]), dtype=np.float32)
    ...
    def insert(self, obs, actions, cent_obs, available_actions=None, active_masks=None):
        for i in range(len(obs)):
            self.obs[self.step] = obs[i].copy()
            self.actions[self.step] = actions[i].copy()
            self.cent_obs[self.step] = cent_obs[i].copy()
            if available_actions is not None:
    ...
    def generator(self, num_mini_batch=None, mini_batch_size=None):
        ...
        cent_obs = self.cent_obs[:self.len].reshape(-1, *self.cent_obs.shape[2:])
        if self.available_actions is not None:
        ...
        for indices in sampler:
            obs_batch = obs[indices]
            actions_batch = actions[indices]
            cent_obs_batch = cent_obs_batch[indices]
            if available_actions is not None:
                available_actions_batch = available_actions[indices]
            else:
                available_actions_batch = None
            ...
            yield obs_batch, actions_batch, cent_obs_batch, available_actions_batch, active_masks_batch
...
from onpolicy.envs.football.Academy_3_vs_1_with_keeper import Academy_3_vs_1_with_Keeper
from onpolicy.envs.football.Academy_counterattack_hard import Academy_Counterattack_Hard
...
#         def init_env():
#             if all_args.env_name == "Football":
#                 if all_args.scenario_name == "academy_3_vs_1_with_keeper":
#                     env = Academy_3_vs_1_with_Keeper(all_args)
#                 elif all_args.scenario_name == "academy_counterattack_hard":
#                     env = Academy_Counterattack_Hard(all_args)
#                 else:
#                     env = FootballEnv(all_args)
#             else:
...
def main(args):
    ...
    all_args.seed = np.random.randint(0, 999)

    assert all_args.scenario_name in ["academy_3_vs_1_with_keeper", "academy_counterattack_hard", "academy_counterattack_easy"], "Current algo only supports several specific scenarios."
    ...
    all_args.experiment_name += "-" + t0 + "--seed-" + str(all_args.seed) + "--state_novel_coef-" + str(all_args.state_novel_coef) + \
                                "--hdd_weight_one-" + str(all_args.use_weight_one) + "--ir_coef-" + str(all_args.ir_coef) + \
                                "--num_objects-" + str(all_args.num_objects)
    ...
    setproctitle.setproctitle(str(all_args.algorithm_name) + "-" + str(all_args.env_name) + "-" + str(all_args.scenario_name) + "-" + str(all_args.experiment_name))
    ...
    if all_args.share_policy:
        ...
        from onpolicy.runner.shared_count.football_runner import FootballRunner as Runner
    else:

...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            elif all_args.env_name == "SparseStarCraft2":
                from onpolicy.envs.starcraft2.Sparse_StarCraft2_Env import SparseStarCraft2Env
                env = SparseStarCraft2Env(all_args)
...
def parse_args(args, parser):
    ...
    parser.add_argument("--use_state_agent", action='store_false', default=False)       # Use env states
...
def main(args):
    ...
    setproctitle.setproctitle(str(all_args.algorithm_name) + "-" + str(all_args.env_name) + "-" + str(all_args.map_name) + "-" + str(all_args.experiment_name))
    ...
    if all_args.env_name == "SMAC":
        ...
        num_entities = get_map_params(all_args.map_name)["n_agents"] + get_map_params(all_args.map_name)["n_enemies"]
        all_args.num_entities = num_entities
    elif all_args.env_name == 'StarCraft2' or all_args.env_name == 'SparseStarCraft2':
        from onpolicy.envs.starcraft2.smac_maps import get_map_params
        num_agents = get_map_params(all_args.map_name)["n_agents"]
        num_entities = get_map_params(all_args.map_name)["n_agents"] + get_map_params(all_args.map_name)["n_enemies"]
        all_args.num_entities = num_entities
    else:
        raise Exception("Unknown settings of env name")
    ...
    assert all_args.share_policy, "SMAC runner is recommended to share policies between agents."

    # run experiments
    if all_args.share_policy:
        # from onpolicy.runner.shared.smac_runner import SMACRunner as Runner
        from onpolicy.runner.shared_count.smac_runner import SMACRunner as Runner
    else:

...
class ReplayBuffer(object):
    def __init__(self, idx, args, obs_space, share_obs_space, act_space):
        ...
        self.state_novels = np.zeros((T + 1, N, 1), dtype=np.float32)

        self.novels = np.zeros(((self.n_agents_in_env + 1), T + 1, N, 1), dtype=np.float32)
    ...
    def compute_door_novel(self, visit_count, lookup_index, pow):
        # Inputs: visit_count contains counts of door.open
        if (visit_count == 0).all():
            return
        visit_count_lookup = self.obs[..., -1:].copy()       # (T+1, N, 1)
        extra_infos = np.ones_like(visit_count_lookup)
        visit_count_lookup = np.concatenate([visit_count_lookup, extra_infos], axis=-1)      # (T+1, N, 2)
        visit_count_lookup = visit_count_lookup.astype(np.int32)
        door_novel = np.power(np.maximum(visit_count[visit_count_lookup[..., 0], visit_count_lookup[..., 1]], 1), -pow)
        self.novels[-1] = door_novel.reshape(self.novels[-1].shape)
    
    def set_all_novels(self, all_novels):
        self.novels[:-1] = all_novels.copy()
...
class Disc(object):
    ...
    def update(self, novels):
        ...
        bounds = novels_[indices][1:-1]     # Sort the novels_, and then select elements according to index (percentile)
        ...
        return (novels_in_bins + 0.5) / self.nbins      # [0, 1]
...
class HindsightReplayBuffer(ReplayBuffer):
    def __init__(self, idx, args, obs_space, share_obs_space, act_space):
        ...
        self.all_states = np.zeros(((self.n_agents_in_env + 1), T + 1, N, 2), dtype=np.int32)
        ...
        if self.disc_na or self.disc_nh:
            self.disc = [Disc(args.discrete_nbins, args.discrete_momentum) for _ in range(self.n_agents_in_env + 1)]
            self.dnovels = np.zeros_like(self.novels)       # It is used to store the discrete version of self.novels.
        ...
        if self.other_reduce:
            ...
            self.all_ratios = np.zeros((T, N, 1), dtype=np.float32)
            self.all_returns_oa = np.zeros_like(self.returns)  # return of all agents in adv
            self.all_returns_oh = np.zeros_like(self.returns)  # return of all agents in ratio
            self.all_returns_oh_reverse = np.zeros_like(self.returns)
        else:
            ...
            self.all_ratios = np.zeros(((self.n_agents_in_env + 1), T, N, 1), dtype=np.float32)
            self.all_returns_oa = np.zeros(((self.n_agents_in_env + 1), T + 1, N, 1), dtype=np.float32)
            self.all_returns_oh = np.zeros_like(self.all_returns_oa)
            self.all_returns_oh_reverse = np.zeros_like(self.all_returns_oa)
        self.hdd_advantages = np.zeros((T, N, 1), dtype=np.float32)
        self.separate_hdd_advantages = np.zeros(((self.n_agents_in_env + 1), T, N, 1), dtype=np.float32)
    ...
    def set_all_novels(self, all_novels):
        ...
        if self.disc is not None:
            for agent_id in range(self.n_agents_in_env):
                ...
                self.dnovels[agent_id] = dnovel.reshape(self.dnovels[agent_id].shape)   # Store discrete version of all agents' novels.

            door_dnovel = self.disc[-1].update(self.novels[-1])
            self.dnovels[-1] = door_dnovel.reshape(self.dnovels[-1].shape)
    
    def compute_reduced_other_novel_returns(self):
        reduced_novels = np.concatenate([self.novels[:self.agent_id], self.novels[self.agent_id+1:]], axis=0).sum(axis=0)   # Note final sum operation, shape=(T+1, N, 1)
        if self.dnovels is not None:
            reduced_dnovels = np.concatenate([self.dnovels[:self.agent_id], self.dnovels[self.agent_id+1:]], axis=0).sum(axis=0)    # shape=(T+1, N, 1)
        ...
        for step in range(self.episode_length):
            ...
            if step == 0:
                ...
                ret = proxy_reward + self.hdd_gamma * self.returns_oh_reverse[step - 1] * self.masks[step]      # r_t + \gamma*return_{t-1}
                if self.hdd_ret_padding:
                    pad_indices = self.masks[step] == 0
                    if pad_indices.sum() > 0:
                        ret[pad_indices] = proxy_reward[pad_indices] / (1 - self.hdd_gamma)
                        
            self.returns_oh_reverse[step] = ret     # I can not figure out why we need to calculate this reversed return.

    def compute_separate_other_novel_returns(self):
        other_novels = np.concatenate([self.novels[:self.agent_id], self.novels[self.agent_id+1:]], axis=0)     # (n_agents-1, T+1, N, 1)
        if self.dnovels is not None:
            other_dnovels = np.concatenate([self.dnovels[:self.agent_id], self.dnovels[self.agent_id+1:]], axis=0)      # (n_agents-1, T+1, N, 1)
        ...
        for step in reversed(range(self.episode_length)):
            ...
            ret = proxy_reward + self.gamma * self.returns_oa[:, step + 1] * self.masks[step + 1]       # T, T-1, T-2, ..., r+\gamma*return
            if self.hdd_ret_padding:
                pad_indices = self.masks[step + 1] == 0
                if pad_indices.sum() > 0:
        ...
        for step in range(self.episode_length):
            ...
            if step == 0:
                ...
                if self.hdd_ret_padding:
                    ...
                    if pad_indices.sum() > 0:
                        ret[:, pad_indices] = proxy_reward[:, pad_indices] / (1 - self.hdd_gamma)
            self.returns_oh_reverse[:, step] = ret      # Does reversed return has any meaning?

    def compute_separate_all_novel_returns(self):
        all_novels = self.novels  # (n_agents+1, T+1, N, 1)
        if self.dnovels is not None:
            all_dnovels = self.dnovels  # (n_agents+1, T+1, N, 1)
        all_na = all_dnovels if self.disc_na else all_novels
        all_nh = all_dnovels if self.disc_nh else all_novels

        if self.hdd_ret_padding:
            # self.returns_oa[:, -1] = other_na[:, -1] / (1 - self.gamma)
            # self.returns_oh[:, -1] = other_nh[:, -1] / (1 - self.hdd_gamma)
            self.all_returns_oa[:, -1] = all_na[:, -10:].mean(axis=1) / (1 - self.gamma)
            self.all_returns_oh[:, -1] = all_nh[:, -10:].mean(axis=1) / (1 - self.hdd_gamma)

        for step in reversed(range(self.episode_length)):
            proxy_reward = all_na[:, step + 1] * self.masks[step + 1]
            ret = proxy_reward + self.gamma * self.all_returns_oa[:, step + 1] * self.masks[step + 1]  # T, T-1, T-2, ..., r+\gamma*return
            if self.hdd_ret_padding:
                pad_indices = self.masks[step + 1] == 0
                if pad_indices.sum() > 0:
                    # ret[:, pad_indices] = other_na[:, step, pad_indices] / (1 - self.gamma)
                    ret[:, pad_indices] = all_na[:, max(0, step - 9): step + 1, pad_indices].mean(axis=1) / (1 - self.gamma)
            self.all_returns_oa[:, step] = ret

            proxy_reward = all_nh[:, step + 1] * self.masks[step + 1]
            ret = proxy_reward + self.hdd_gamma * self.all_returns_oh[:, step + 1] * self.masks[step + 1]
            if self.hdd_ret_padding:
                pad_indices = self.masks[step + 1] == 0
                if pad_indices.sum() > 0:
                    # ret[:, pad_indices] = other_nh[:, step, pad_indices] / (1 - self.hdd_gamma)
                    ret[:, pad_indices] = all_nh[:, max(0, step - 9): step + 1, pad_indices].mean(axis=1) / (1 - self.hdd_gamma)
            self.all_returns_oh[:, step] = ret

        for step in range(self.episode_length):
            proxy_reward = all_nh[:, step].copy()
            if step == 0:
                ret = proxy_reward
                if self.hdd_ret_padding:
                    ret = proxy_reward / (1 - self.hdd_gamma)
            else:
                ret = proxy_reward + self.hdd_gamma * self.all_returns_oh_reverse[:, step - 1] * self.masks[step]
                if self.hdd_ret_padding:
                    pad_indices = self.masks[step] == 0
                    if pad_indices.sum() > 0:
                        ret[:, pad_indices] = proxy_reward[:, pad_indices] / (1 - self.hdd_gamma)
            self.all_returns_oh_reverse[:, step] = ret  # Does reversed return has any meaning?

    def compute_other_novel_returns(self):
        # Maybe, the first one directly summarizes up all agents's rewards and subsequently calculate the coresponding cumulative return.
        # The second one respectively calculates their cumulative returns?
        if self.other_reduce:
            self.compute_reduced_other_novel_returns()
        else:
            self.compute_separate_other_novel_returns()

    def compute_all_novel_returns(self):
        assert not self.other_reduce
        self.compute_separate_all_novel_returns()
    
    def compute_returns(self, self_coef=0.0, other_coef=0.0, value_normalizer=None, hdd_coef=0.0, novel_max=False, use_state_count=False):
        if self._use_gae:
            gae = 0.0
            for step in reversed(range(self.episode_length)):
                proxy_reward = self.ext_rewards[step].copy()
                if use_state_count:
                    proxy_reward += self_coef * self.state_novels[step + 1] * self.masks[step + 1]
                else:
                    if novel_max:
                        proxy_reward += self_coef * self.novels[:-1, step + 1].max(axis=0) * self.masks[step + 1]
                    else:
                        proxy_reward += self_coef * self.novels[self.agent_id, step + 1] * self.masks[step + 1]
                        proxy_reward += other_coef * (self.novels[:-1, step + 1].sum(axis=0) -
                                                      self.novels[self.agent_id, step + 1]) * self.masks[step + 1]
                # Note: we only consider the mutual-information intrinsic corresponding to current selected skills.
                proxy_reward += hdd_coef * self.hdd_advantages[step]   # proxy_r=task_r+local_novelty+others_novelty+hdd_r(all objects)
                self.int_rewards[step] = proxy_reward - self.ext_rewards[step]

...
class Runner(Base):
    ...
    def run(self):
        ...
        # eval_rewards, best_eval_reward, best_epoch = [], -999999, 0
        ...
        for epoch in range(epoches):
            ...
            self.old_state_visit_counts = deepcopy(self.state_visit_counts)
            self.visit_counts = deepcopy(self.envs.get_visit_counts())
            self.state_visit_counts = deepcopy(self.envs.get_state_visit_counts())

            # remove count of next observation at last step
            # state_visit_counts.shape=(grid_size, grid_size, grid_size, grid_size, grid_size, grid_size)
            row, col = self.state_visit_counts.shape[0], self.state_visit_counts.shape[1]
            obs_ = np.zeros_like(obs[..., :2])  # shape=(n_rollout_threads, num_agents, [x, y])
            obs_[..., 0] = obs[..., 0] * row  # Recover original x coordinate. Obs needs to / grid_size
            obs_[..., 1] = obs[..., 1] * col  # Recover original y coordinate.
            obs_ = obs_.reshape(self.n_rollout_threads, -1).astype(np.int32)  # (n_rollout_threads, 4 including [agent1.x, agent1.y, agent2.x, agent2.y])
            for i, o in enumerate(obs_):
                if self.use_door_in_state:
                    index = tuple(o) + (int(obs[i, 0, -1]), 1)
                else:
                    index = tuple(o)
                self.state_visit_counts[index] -= 1

            if self.novel_cen:
                # visit_counts.shape=(grid_size, grid_size, grid_size, grid_size)
                row, col = self.visit_counts.shape[0], self.visit_counts.shape[1]
                obs_ = np.zeros_like(obs[..., :2])      # shape=(n_rollout_threads, num_agents, [x, y])
                obs_[..., 0] = obs[..., 0] * row        # Recover original x coordinate. Obs needs to / grid_size
                obs_[..., 1] = obs[..., 1] * col        # Recover original y coordinate.
                obs_ = obs_.reshape(self.n_rollout_threads, -1).astype(np.int32)    # (n_rollout_threads, agent1.x, agent1.y, agent2.x, agent2.y)
                for i, o in enumerate(obs_):
                    index = tuple(o) + (int(obs[i, 0, -1]), 1)
                ...
                for agent_id in range(self.num_agents):
                    ...
                    obs_ = obs[:, agent_id]     # shape=(n_rollout_threads, [x, y, door])
                    for o in obs_:
                        x = o[0] * row
                        y = o[1] * col
                        self.visit_counts[agent_id, int(x), int(y)] -= 1
                for o in obs[:, 0]:     # obs[:, 0].shape=(n_rollout_threads, obs_shape)
                    self.visit_counts[2, int(o[-1]), 1] -= 1

            # What is the purpose? Maybe only for consider using count_decay_coef.
            self.curr_visit_counts += self.visit_counts - self.old_visit_counts * self.count_decay_coef

            train_infos = self.compute()

            # Note: 2024-5-20: add storage of obs, state_novelty and hdd_advantage
            # if epoch % 50 == 0 or epoch == (epoches - 1):
            #     np.save(self.run_dir / "agent_0_epoch_{}_timestep_{}_obs.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[0].obs)
            #     np.save(self.run_dir / "agent_0_epoch_{}_timestep_{}_state_novelty.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[0].state_novels)
            #     np.save(self.run_dir / "agent_0_epoch_{}_timestep_{}_hdd_advantage.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[0].hdd_advantages)
            #
            #     np.save(self.run_dir / "agent_1_epoch_{}_timestep_{}_obs.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[1].obs)
            #     np.save(self.run_dir / "agent_1_epoch_{}_timestep_{}_state_novelty.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[1].state_novels)
            #     np.save(self.run_dir / "agent_1_epoch_{}_timestep_{}_hdd_advantage.npy".format(epoch, (epoch + 1) * self.episode_length * self.n_rollout_threads), self.buffer[1].hdd_advantages)
            ...
            if epoch % self.save_interval == 0 or epoch == (epoches - 1):
                self.save(epoch)
            ...
            if epoch % self.log_interval == 0:
                ...
                for agent_id in range(self.num_agents):
                    ...
                    train_infos[agent_id].update({"all_novelties": np.mean(self.buffer[agent_id].novels)})
                    train_infos[agent_id].update({"all_hdd_rew": np.mean(self.buffer[agent_id].separate_hdd_advantages)})
                    train_infos[agent_id].update({"hdd_rew": np.mean(self.buffer[agent_id].hdd_advantages)})
                    train_infos[agent_id].update({"state_novelties": np.mean(self.buffer[agent_id].state_novels)})
            ...
            if epoch % self.eval_interval == 0 and self.use_eval:
                eval_reward = self.eval_reward(total_num_steps)
    ...
    def warmup(self):
        ...
        self.state_visit_counts = self.envs.get_state_visit_counts()
        self.old_state_visit_counts = deepcopy(self.state_visit_counts)
    ...
    @torch.no_grad()
    def eval_reward(self, total_num_steps=None, render=0.0):
        ...
        for t in range(self.episode_length):
            ...
            for agent_id in range(self.num_agents):
                ...
                if self.eval_envs.action_space[agent_id].__class__.__name__ == 'MultiDiscrete':
                    ...
                    eval_action_env = np.clip(eval_action, self.eval_envs.action_space[agent_id].low,
                                              self.eval_envs.action_space[agent_id].high)
                else:
                    raise NotImplementedError
        ...
        for agent_id in range(self.num_agents):
            ...
            print(
                f"eval episode rewards of agent{agent_id}: {eval_average_episode_rewards:.4f} ± {eval_std_episode_rewards:.4f}")

...
from src.utils.buffer import HindsightBuffer, HindsightReplayBuffer, ReplayBuffer
...
class Base(object):
    def __init__(self, config):
        ...
        self.use_state_count = self.all_args.use_state_count
        self.use_door_in_state = self.all_args.use_door_in_state

        self.use_state_mutual = self.all_args.use_state_mutual
        ...
        for agent_id in range(self.num_agents):
            ...
            # policy network
            if self.all_args.algorithm_name in ["rmappo", "mappo"]:
                local_policy = R_MAPPOPolicy(self.all_args,
                                             self.envs.observation_space[agent_id],
                                             share_observation_space,
                                             self.envs.action_space[agent_id],
                                             device=self.device)
                self.policy.append(local_policy)
            else:
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.all_args.algorithm_name in ["rmappo", "mappo"]:
                local_trainer = R_MAPPO(self.all_args, self.policy[agent_id], device=self.device)
            else:
                raise Exception("Unknown algorithm.")
            ...
            self.trainer.append(local_trainer)
        ...
        for agent_id in range(self.num_agents):
            if self.policy[agent_id].use_hdd and not self.policy[agent_id].hdd_count:
                # The buffer_hdd is only used for network-based hdd calculation.
                if self.all_args.hdd_reduce:
                    ...
                    self.buffer_hdd.append([HindsightBuffer(self.all_args,
                                                            self.envs.observation_space[agent_id],
                                                            self.envs.action_space[agent_id]) for _ in range(self.num_agents - 1)])
            else:
    ...
    def compute(self):
        if self.novel_type == 1:
            ...
            if self.count_prev_epoch:
                ...
                state_visit_counts = self.old_state_visit_counts.copy()
            else:
                visit_counts = self.visit_counts.copy()     # Extract from the env's recordings.
                state_visit_counts = self.state_visit_counts.copy()

            # Shape novelty-based intrinsic to each agent's buffer.novels. Subsequently we need to exchange / communicate between agents
            calculate_cen_novel(self.buffer, state_visit_counts, self.count_lookup_index, self.count_pow, self.use_door_in_state)

            # assert not self.novel_cen, "Current algo respectively calculates global state novelty and object-oriented state novelty."
            if self.novel_cen:
                ...
                for agent_id in range(self.num_agents):
                    self.buffer[agent_id].compute_novel(visit_counts[agent_id], 
                                                        ...
                                                        self.count_pow)
                    self.buffer[agent_id].compute_door_novel(visit_counts[2],
                                                             self.count_lookup_index,
                                                             self.count_pow)        # Here visit_counts[2] means door.open.
        ...
        # Shape=(n_agents, T+1, N, 1), should be stored into per-agent buffer and sum up as approximated global novelty
        # Here door_novel is already stored into per-agent buffer, which do not need extra operations.
        all_novels = np.stack([self.buffer[i].novels[i] for i in range(self.num_agents)], axis=0)

        agent_states = np.stack([(self.buffer[i].obs[..., :2] * 30) for i in range(self.num_agents)], axis=0)    # shape=(n_agents, T+1, N, 2)
        door_states = np.concatenate([self.buffer[0].obs[..., -1:], np.ones_like(self.buffer[0].obs[..., -1:])], axis=-1)  # (T+1, N, 2)
        all_states = np.concatenate([agent_states, np.expand_dims(door_states, axis=0)], axis=0)    # (n_agents+1, T+1, N, 2)

        infos = [{} for _ in range(self.num_agents)]

        for agent_id in range(self.num_agents):
            self.buffer[agent_id].set_all_novels(all_novels)
            self.buffer[agent_id].compute_all_novel_returns()

            self.buffer[agent_id].all_states = all_states.copy()

            # 这里我们原本担心会出现不同skill的hdd_tables混用的情况，但发现hdd_tables和policy是1对1关系，而且当前hdd_update仅用当前policy rollout的数据进行更新，所以不存在混用问题.
            if self.use_state_mutual:
                info = self.trainer[agent_id].state_hdd_update(self.buffer[agent_id], self.buffer_hdd[agent_id])
            else:
                info = self.trainer[agent_id].hdd_update(self.buffer[agent_id], self.buffer_hdd[agent_id])      # Refresh the counting table in policy.hdd_table.

            for k, v in info.items():
                if len(v) > 0:
                    infos[agent_id][k] = np.mean(v)

            self.trainer[agent_id].cal_hdd_advantage(self.buffer[agent_id])     # (weights*returns).sum() as hdd intrinsic reward.
            ...
            self.buffer[agent_id].compute_returns(self.trainer[agent_id].self_coef,
                                                  ...
                                                  use_state_count=self.use_state_count)
    ...
    def save(self, episode=None):
        ...
        for agent_id in range(self.num_agents):
            ...
            torch.save(actor.state_dict(), str(save_dir) + "/actor_agent_{}.pt".format(agent_id))
            critic = self.trainer[agent_id].policy.critic
            torch.save(critic.state_dict(), str(save_dir) + "/critic_agent_{}.pt".format(agent_id))
    ...
    def restore(self):
        for agent_id in range(self.num_agents):
            actor_state_dict = torch.load(str(self.model_dir) + '/actor_agent_{}.pt'.format(agent_id))
            self.policy[agent_id].actor.load_state_dict(actor_state_dict)
            critic_state_dict = torch.load(str(self.model_dir) + '/critic_agent_{}.pt'.format(agent_id))
        ...
        print("Finished loading pre-trained models.")
...
def calculate_cen_novel(buffers, visit_count, index, pow, use_door_in_state):
    ...
    visit_count_lookup = np.concatenate(visit_count_lookup, axis=-1)  # shape=(T+1, N, 2*2)
    if use_door_in_state:
        extra_inps = np.concatenate([buffers[0].obs[..., -1:], np.ones_like(buffers[0].obs[..., -1:])], axis=-1).astype(np.int32)  # shape=(T+1, N, 2)
        visit_count_lookup = np.concatenate([visit_count_lookup, extra_inps], axis=-1)      # shape(T+1, N, 2+2+2)
    visit_count_lookup = tuple([visit_count_lookup[..., i:i + 1] for i in range(visit_count_lookup.shape[-1])])
    state_novel = np.power(np.maximum(visit_count[visit_count_lookup], 1), -pow)  # Novelty=1/np.sqrt(N(global_s))
    for agent_id, buf in enumerate(buffers):
        buf.state_novels = state_novel.copy()  # All agents share the same global state novelty.
...
def compute_cen_novel(buffers, visit_count, index, pow):
    ...
    visit_count_lookup = np.concatenate([visit_count_lookup, extra_inps], axis=-1)  # shape(T+1, N, 2+2+2)
    visit_count_lookup = tuple([visit_count_lookup[..., i:i+1] for i in range(visit_count_lookup.shape[-1])])
    novel = np.power(np.maximum(visit_count[visit_count_lookup], 1), -pow)      # Novelty=1/np.sqrt(N(global_s))
    for agent_id, buf in enumerate(buffers):
        buf.novels[agent_id] = novel.copy()     # All agents share the same global state novelty.
...
class R_MAPPO(object):
    def __init__(self, args, policy, device=torch.device("cpu")):
        ...
        self.use_state_mutual = args.use_state_mutual
    ...
    def ppo_update(self, sample):
        ...
        return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch = sample
    ...
    def hdd_update(self, buffer, buffer_hdd):
        ...
        if buffer.dnovels is None:
            rewards_o = buffer.novels[:, :-1]       # (n_agents+1, T, N, 1)
        else:
            rewards_o = buffer.dnovels[:, :-1]      # (n_agents+1, T, N, 1)

        if self.hdd_reduce:
            rewards_o = np.sum(rewards_o, axis=0)       # (T, N, 1)
            returns_oh = buffer.all_returns_oh[:-1]         # (T, N, 1)
            returns_oh_reverse = buffer.all_returns_oh_reverse[:-1]     # (T, N, 1)
        else:
            returns_oh = buffer.all_returns_oh[:, :-1]      # (n_agents+1, T, N, 1)
            returns_oh_reverse = buffer.all_returns_oh_reverse[:, :-1]      # (n_agents+1, T, N, 1)
    ...
    def state_hdd_update(self, buffer, buffer_hdd):
        if not self.use_hdd:
            return {}
        ...
        assert not self.hdd_reduce

        # all_states = buffer.all_states[:, :-1]      # (n_agents+1, T, N, 2)

        # Here all_states should locate in t+1 time step, to calculate I(s_{t+1}^{j}; a_{t}^{i}|o_{t}^{i})
        all_states = buffer.all_states[:, 1:]       # (n_agents+1, T, N, 2)
        returns_oh_reverse = buffer.all_returns_oh_reverse[:, :-1]  # (n_agents+1, T, N, 1)
        ...
        if buffer.available_actions is None:
            ...
            available_actions = buffer.available_actions[:-1]
        if self.hdd_count:
            return self.policy.hdd.update(buffer.obs[:-1], buffer.actions, all_states, rewards_o,
                                          returns_oh_reverse, available_actions, buffer.active_masks[:-1])
        return self.policy.hdd.update(buffer_hdd, buffer.obs[:-1], buffer.actions, all_states, rewards_o,
                                      returns_oh_reverse, available_actions, buffer.active_masks[:-1])
    ...
    def cal_hdd_ratio(self, buffer):
        ...
        h_probs = self.policy.hdd.evaluate_actions(buffer.obs[:-1], buffer.actions, returns_oh, rewards_o,
                                                   returns_oh_reverse, buffer.available_actions[:-1])       # p(a^i|o^i,z^j)=n/N, shape=(n_agents+1, T, N, 1)
        a_probs = np.exp(buffer.action_log_probs)       # p(a^i|o^i), shape=(T, N, 1)
        if self.hdd_ratio_clip is not None:
            buffer.all_ratios[:] = np.clip(a_probs / h_probs, 0., self.hdd_ratio_clip)
        else:
            buffer.all_ratios[:] = a_probs / h_probs        # Here why a_probs / h_pros?
    ...
    def cal_state_hdd_ratio(self, buffer):
        ...
        # all_states = buffer.all_states[:, :-1]      # (n_agents+1, T, N, 1)

        # Here we use all entities' states at next time step t+1.
        all_states = buffer.all_states[:, 1:]
        returns_oh_reverse = buffer.all_returns_oh_reverse[:, :-1]  # (n_agents+1, T, N, 1)

        h_probs = self.policy.hdd.evaluate_actions(buffer.obs[:-1], buffer.actions, all_states, rewards_o,
                                                   returns_oh_reverse, buffer.available_actions[:-1])       # p(a^i|o^i,s^j)=n/N, shape=(n_agents+1, T, N, 1)
        a_probs = np.exp(buffer.action_log_probs)       # p(a^i|o^i), shape=(T, N, 1)
        if self.hdd_ratio_clip is not None:
            buffer.all_ratios[:] = np.clip(a_probs / h_probs, 0., self.hdd_ratio_clip)
        else:
    ...
    def cal_hdd_advantage(self, buffer):
        ...
        if self.use_state_mutual:
            ...
            self.cal_hdd_ratio(buffer)      # Calculate a_probs / h_probs, and save it to self.all_ratios
        
        weight = -np.log(buffer.all_ratios + 1e-6) if self.hdd_log_weight else (1 - buffer.all_ratios)      # log(h_probs / a_probs), (n_agents+1, T, N, 1)
        ...
        if self.hdd_weight_only:
            ...
            # returns = buffer.all_returns_oa[:-1] if self.hdd_reduce else buffer.all_returns_oa[:, :-1]  # (T, N, 1) if self.hdd_reduce else (n_agents+1, T, N, 1)
            # hdd_advantages = weight * returns  # shape=(n_agents+1, T, N, 1)

            # 2024-4-5: use immediate novels to calculate hdd_advantages
            # 2024-4-6: Note here we use the cumulative return of s_{t+1}^{j}
            novel_returns = buffer.novels[:, 1:]
            hdd_advantages = weight * novel_returns

            # Use immediate dnovels
            # dnovel_returns = buffer.dnovels[:, 1:]
            # hdd_advantages = weight * dnovel_returns

        if self._use_hdd_active_masks:
            hdd_advantages *= buffer.active_masks[:-1]
        
        buffer.hdd_advantages = hdd_advantages.copy() if self.hdd_reduce else hdd_advantages.sum(axis=0)        # Respectively calculate mutual_intrinsic*return, then sum up.
        assert not self.hdd_reduce
        buffer.separate_hdd_advantages = hdd_advantages.copy()      # (n_agents+1, T, N, 1)

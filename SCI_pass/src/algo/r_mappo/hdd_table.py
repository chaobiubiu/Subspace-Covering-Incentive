...
class HDDTable(object):
    def __init__(self, act_space, args):
        ...
        count_shape += (self.ret_nbins,)        # count_shape=(act_n, obs_shape, K)
        
        if self.wsize > 1:
            count_shape = (self.wsize,) + count_shape   # count_shape=(n_windows, act_n, obs_shape, K), should introduce a counting indicator to periodically reset the counting table.
            self.widx = 0
    ...
    def update(self, obs, actions, returns, last_rewards=None, reverse_returns=None):
        ...
        ret_ind = np.digitize(returns, self.ret_bound) - 1      # Discretize the return into K bins
        indices = np.concatenate([act_ind, obs_ind, ret_ind], axis=-1)      # [a^i, o^i, Discretize(return z^j)]
    ...
    def predict(self, obs, actions, returns, last_rewards=None, reverse_returns=None):
        ...
        if self.wsize > 1:
            # Need to sum up axis=0 [n_windows]. Sum up all possible act and index by [obs_n, Discretize(return z^j)]
            return self.hdd.sum(axis=0)[indices] / self.hdd.sum(axis=(0, 1))[indices[1:]]
        return self.hdd[indices] / self.hdd.sum(axis=0)[indices[1:]]        # When wsize=1, axis=0 refers to act_n. Sum up all possible act and index by [obs_n, Discretize(return z^j)]
...
class HDDTables(object):
    def __init__(self, act_space, args):
        ...
        if self.reduce:
            ...
            self.hdd = [HDDTable(act_space, args) for _ in range(self.n_agents + 1)]
    ...
    def update(self, obs, actions, returns, last_rewards=None, reverse_returns=None,
        ...
        if self.reduce:
            ...
            for i in range(self.n_agents + 1):     # Store a^i, o^i, z^j, also include its own z^i and the door's z^i.

...
from src.algo.r_mappo.state_hdd_table import StateHDDTables
...
class R_MAPPOPolicy:
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        if self.use_hdd:
            if self.hdd_count:
                if args.use_state_mutual:
                    self.hdd = StateHDDTables(self.act_space, args)     # \omega*|O|*|A|*|local S|
                else:
                    self.hdd = HDDTables(self.act_space, args)      # shape should be \omega*|S|*|A|*|discrete bins|?
            else:

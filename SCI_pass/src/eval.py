...
def parse_args(args, parser):
    ...
    parser.add_argument("--use_hierarchical", action="store_true", help="whether to use the hierarchical paradigm.")
    parser.add_argument("--epoch_ratio", type=float, default=0.0, help="the percentile of epoches used to finetune.")
    parser.add_argument("--step_interval", type=int, default=10, help="the decision interval of the high-level policy.")
...
def main(args):
    ...
    all_args.seed = np.random.randint(0, 99)

    # check recurrent setting
    if all_args.algorithm_name == "rmappo":
        assert (all_args.use_recurrent_policy or all_args.use_naive_recurrent_policy), ("check recurrent policy!")
    elif all_args.algorithm_name == "sci":
        assert (all_args.use_recurrent_policy == True), "check recurrent policy used!"
    ...
    all_args.experiment_name += "-" + t0 + "-seed" + str(all_args.seed) + "--ir_coef" + str(all_args.ir_coef)
    ...
    for obs_space, act_space, share_obs_space in zip(envs.observation_space, envs.action_space,
                                                     envs.share_observation_space):

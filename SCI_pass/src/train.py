...
def make_grid_env(all_args):
    config = {
        ...
        "use_state_count": all_args.use_state_count,
        "use_door_in_state": all_args.use_door_in_state
    }
...
def parse_args(args, parser):
    ...
    parser.add_argument("--use_state_count", action="store_true", help="whether use the state count or sum of local novelty")
    parser.add_argument("--use_door_in_state", action="store_true", help="whether include the door in the state")

    parser.add_argument("--use_state_mutual", action="store_true", help="whether calculate I(s; a|o)")
...
def main(args):
    ...
    all_args.seed = np.random.randint(0, 599)
    ...
    all_args.experiment_name += "-" + t0 + "-seed-" + str(all_args.seed) +"--self_coef-" + str(all_args.self_coef) +\
                                "--other_coef-" + str(all_args.other_coef) + "--ir_coef-" + str(all_args.ir_coef) +\
                                "--count_state-" + str(all_args.use_state_count) + "--door_in_state-" + str(all_args.use_door_in_state) +\
                                "--use_state_mutual-" + str(all_args.use_state_mutual) + "--n_rollout_threads-" + str(all_args.n_rollout_threads) + \
                                "--hdd_weight_one" + str(all_args.hdd_weight_one)
    ...
    for obs_space, act_space, share_obs_space in zip(envs.observation_space, envs.action_space, envs.share_observation_space):
        print("obs space:", obs_space, "; act space:", act_space, ";share obs space:", share_obs_space)
    ...
    if all_args.novel_type == 3:  # discrete, Count-based
        from src.runner.runner_grid import Runner
    elif all_args.novel_type == 4:  # continous, RND

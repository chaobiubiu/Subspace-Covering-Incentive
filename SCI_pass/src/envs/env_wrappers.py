    ...
    while True:
        ...
        # 4 commands for gridworld
            ...
            if hasattr(env, 'visit_counts'):
                ...
                raise NotImplementedError
        elif cmd == 'get_state_visit_counts':
            if hasattr(env, 'state_visit_counts'):
                remote.send(env.get_state_visit_counts())
            elif hasattr(env.unwrapped, 'state_visit_counts'):
                remote.send(env.unwrapped.get_state_visit_counts())
            else:
                raise NotImplementedError
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def get_state_visit_counts(self):
        for remote in self.remotes:
            remote.send(('get_state_visit_counts'))
        return sum(remote.recv() for remote in self.remotes)
...
class DummyVecEnv(ShareVecEnv):
    ...
    def get_state_visit_counts(self):
        if hasattr(self.envs[0], 'state_visit_counts'):
            return sum(env.get_state_visit_counts() for env in self.envs)
        else:
            return sum(env.unwrapped.get_state_visit_counts() for env in self.envs)

...
class Pass(gym.Env):
    def __init__(self, map_ind=0, max_timesteps=300, door_in_obs=False, full_obs=False, joint_count=False, 
                 activate_radius=None, grid_size=30, use_state_count=False, use_door_in_state=False, **kwargs):
        self.num_agents = 2
        ...
        self.use_state_count = use_state_count
        self.use_door_in_state = use_door_in_state
        ...
        if self.use_door_in_state:
            self.state_visit_counts = np.zeros((self.num_agents + 1) * [self.grid_size, self.grid_size])    # [grid_size, grid_size, grid_size, grid_size, grid_size, grid_size]
        else:
            self.state_visit_counts = np.zeros(self.num_agents * [self.grid_size, self.grid_size])      # [grid_size, grid_size, grid_size, grid_size]

        if self.joint_count:
            self.visit_counts = np.zeros((self.num_agents + 1) * [self.grid_size, self.grid_size])    # [grid_size, grid_size, grid_size, grid_size, grid_size, grid_size]
        else:
            self.visit_counts = np.zeros(((self.num_agents + 1), self.grid_size, self.grid_size))     # [num_agents+1, grid_size, grid_size]
        self.reset()
    ...
    def reset(self):
        ...
        if self.use_door_in_state:
            visit_indices += (int(self.door.open), 1)
        self.state_visit_counts[visit_indices] += 1
        ...
        if self.joint_count:
            ...
            self.visit_counts[2, int(self.door.open), 1] += 1
    ...
    def _get_obs(self):
        # Note here agent's x, y coordinate / grid size
    ...
    def get_state_visit_counts(self):
    ...
    def reset_visit_counts(self):
        ...
        if self.joint_count:
            ...
            self.visit_counts = np.zeros(((self.num_agents + 1), self.grid_size, self.grid_size))
